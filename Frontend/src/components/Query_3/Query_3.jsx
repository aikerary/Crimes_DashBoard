...
const BarChart = () => {
  ...
  const fetchData = async (crimeDesc) => {
    try {
      const response = await fetch(`http://localhost:5000/get_crime_count_and_hours/${crimeDesc}`);
    ...
    <div>
      ...
      <div>
        ...
        <select id="crimeSelect" value={selectedCrime} onChange={handleCrimeChange}>
          ...
          <option value="ARSON">INCENDIO PROVOCADO</option>
<option value="ASSAULT WITH DEADLY WEAPON ON POLICE OFFICER">AGRESIÓN CON ARMA MORTAL A UN AGENTE DE POLICÍA</option>
